...
def after_install():
    ...
    create_default_gift_certificate_type()


def create_default_gift_certificate_type():
    """
    Create the default "Standard" Gift Certificate Type so the Gift Certificate
    Print Format has a template to render against out of the box.
    """
    try:
        if frappe.db.exists("Gift Certificate Type", "Standard"):
            frappe.logger().info("Gift Certificate Type already exists: Standard")
            return
        ...
        frappe.db.commit()
        frappe.logger().info("Created default Gift Certificate Type: Standard")

    except Exception:
        frappe.logger().error("Error creating default Gift Certificate Type")
        frappe.log_error(frappe.get_traceback(), "Gift Certificate Type Creation Error")
